...
body.push(bullet([new TextRun({ text: 'server/ ', bold: true }), new TextRun('— the Express JSON API: db.js (one connection pool), config.js, middleware.js (role guards), and routes for session, menu, and orders.')]));
...
body.push(p('render.yaml is a Render blueprint that creates a free Postgres instance (chowly-db) and a free Node web service (chowly). The build command installs both workspaces and builds client/dist; the start command runs the server. DATABASE_URL is injected from the database, SESSION_SECRET is generated by Render, and NODE_ENV is set to production. The server sets "trust proxy" and a secure session cookie for HTTPS behind Render’s proxy, and the database layer turns on SSL in production.'));
...
body.push(nDeploy('Open the chowly service → Shell and run node db/setup.mjs once. This applies schema.sql, seed.sql and menu_images.sql (it uses the pg driver, so no psql binary is needed on the host).'));
body.push(nDeploy('Open the service URL. Every later push to the default branch redeploys automatically; the database keeps its data.'));
body.push(...shot('The Render Blueprint creating chowly-db and the chowly web service'));
body.push(...shot('node db/setup.mjs run once in the Render Shell'));
