...
body.push(lead('Vercel ', '(vercel.json): client/ is built and served as static output; /api/* is rewritten to the api/serverless.js function; everything else falls back to index.html. DATABASE_URL (a Neon pooled connection string) and SESSION_SECRET are set in the Vercel project; NODE_ENV=production is automatic.'));
body.push(p('To deploy to Vercel: create a free Neon project and copy its pooled connection string; import the GitHub repo at vercel.com/new; add DATABASE_URL and SESSION_SECRET; deploy. The first request loads the schema and seed, then it is instant. To reset either database by hand, run node db/setup.mjs from a laptop against that database’s connection string.'));
