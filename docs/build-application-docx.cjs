...
body.push(lead('Deployed application: ', '____________________  (paste your Render URL)'));
body.push(p(
...
body.push(bullet([new TextRun({ text: 'Hosting — ', bold: true }), new TextRun('Render: one managed Postgres instance and one Node web service, wired together by render.yaml.')]));
...
body.push(p('In development, one command runs the API on port 3000 and the Vite dev server on port 5173, with Vite proxying /api to Express. In production, the build compiles the client to client/dist and Express serves it with a single-page-app fallback, so the whole application is served from one origin.'));
...
body.push(bullet([new TextRun({ text: 'server/ ', bold: true }), new TextRun('— the Express JSON API: db.js (one connection pool), bootstrap.js (first-boot schema load), config.js, middleware.js (role guards), and routes for session, menu, and orders.')]));
...
body.push(bullet([new TextRun({ text: 'render.yaml, docs/ ', bold: true }), new TextRun('— the Render blueprint and this document.')]));
...
body.push(p('render.yaml is a Render blueprint that creates a free Postgres instance (chowly-db) and a free Node web service (chowly), both in the same region. DATABASE_URL is injected from the database, SESSION_SECRET is generated by Render, and NODE_ENV is set to production. The build command runs "npm install --omit=dev" for the root and server and "npm install --include=dev" for the client before "vite build"; the --include=dev is necessary because Render sets NODE_ENV=production during the build, which would otherwise skip Vite and Tailwind. The health check is a plain liveness check that does not touch the database, so a slow first-boot database cannot fail the deploy. The server sets "trust proxy" and a secure session cookie for HTTPS behind Render’s proxy, and the database layer turns on SSL in production.'));
body.push(p('The free tier has no Shell, so server/bootstrap.js loads schema.sql, seed.sql and menu_images.sql automatically the first time the server starts against an empty database. It checks for the menu_item table and is a no-op once it exists, so redeploys never touch data.'));
body.push(p('The steps to deploy:'));
body.push(nDeploy('Push the repository to GitHub.'));
body.push(nDeploy('In the Render dashboard, choose New + → Blueprint and pick the repository. Render reads render.yaml and creates chowly-db and the chowly web service. Click Apply and wait for the first deploy to go green.'));
body.push(nDeploy('Open the service URL. The database loads itself on first boot. Every later push to the default branch redeploys automatically; the database keeps its data.'));
body.push(p('To reset or reload the hosted data by hand, run node db/setup.mjs from a laptop against the database’s external connection string.'));
body.push(...shot('The Render Blueprint creating chowly-db and the chowly web service'));
body.push(...shot('The first-boot logs — bootstrap loading schema, seed and menu images'));
body.push(...shot('The deployed application open at its Render URL'));
...
body.push(bullet('Later feature requests taken as-is: a welcome landing page; order lookup by name and table; a live MM:SS countdown; the logo linking home; the table picker as a 1–20 dropdown; the waiter opening a paid card for a full summary; optional dish photos.'));
