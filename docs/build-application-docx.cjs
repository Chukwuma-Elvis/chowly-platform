...
body.push(lead('Vercel ', '(vercel.json): client/ is built and served as static output; /api/* is rewritten to the api/serverless.js function; everything else falls back to index.html. The Neon database is created from the Vercel dashboard (Storage tab, no separate Neon account) and a family of connection variables is injected automatically; db.js takes an explicit DATABASE_URL if present and otherwise scans the environment for the best postgres:// URL (preferring the pooled, TLS one), so the integration’s generated names work as-is. SESSION_SECRET is added by hand; NODE_ENV=production is automatic.'));
